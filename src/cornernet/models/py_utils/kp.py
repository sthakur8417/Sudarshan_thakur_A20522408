...
from .utils import mklayer, mklayer_revr

from .kp_utils import _tranpose_and_gather_feat, _decode
from .kp_utils import _sigmoid, _ae_loss, _regr_loss, _neg_loss
from .kp_utils import mktlLy, mkbrLy, mkkpLy
from .kp_utils import mkpoolLy, mkunpoolLy
from .kp_utils import mkmergeLy, mkinterLy, mkcnvLy

class kp_module(nn.Module):
    def __init__(
        self, n, dims, modules, layer=residual,
        mkupLy=mklayer, mklowLy=mklayer,
        mkhgLy=mklayer, mkhgLy_revr=mklayer_revr,
        mkpoolLy=mkpoolLy, mkunpoolLy=mkunpoolLy,
        mkmergeLy=mkmergeLy, **kwargs
    ):
        ...
        self.low2 = kp_module(
            ...
            mklowLy=mklowLy,
            mkhgLy=mkhgLy,
            mkhgLy_revr=mkhgLy_revr,
            mkpoolLy=mkpoolLy,
            mkunpoolLy=mkunpoolLy,
            mkmergeLy=mkmergeLy,
            **kwargs
        ) if self.n > 1 else \
        mklowLy(
        ...
        self.low3 = mkhgLy_revr(
            3, next_dim, curr_dim, curr_mod,
            layer=layer, **kwargs
        )
        self.up2  = mkunpoolLy(curr_dim)

        self.merge = mkmergeLy(curr_dim)
...
class kp(nn.Module):
    def __init__(
        ...
        mktlLy=mktlLy, mkbrLy=mkbrLy,
        mkcnvLy=mkcnvLy, mkheatLy=mkkpLy,
        mktagLy=mkkpLy, mkregrLy=mkkpLy,
        mkupLy=mklayer, mklowLy=mklayer, 
        mkhgLy=mklayer, mkhgLy_revr=mklayer_revr,
        mkpoolLy=mkpoolLy, mkunpoolLy=mkunpoolLy,
        mkmergeLy=mkmergeLy, mkinterLy=mkinterLy, 
        kpLy=residual
    ):
        ...
        self.cnvs = nn.ModuleList([
            mkcnvLy(curr_dim, cnv_dim) for _ in range(nstack)
        ])

        self.tl_cnvs = nn.ModuleList([
            mktlLy(cnv_dim) for _ in range(nstack)
        ])
        self.br_cnvs = nn.ModuleList([
            mkbrLy(cnv_dim) for _ in range(nstack)
        ])


        self.tl_heats = nn.ModuleList([
            mkheatLy(cnv_dim, curr_dim, out_dim) for _ in range(nstack)
        ])
        ...
        self.tl_tags  = nn.ModuleList([
            mktagLy(cnv_dim, curr_dim, 1) for _ in range(nstack)
        ])
        ...
        self.inters = nn.ModuleList([
            mkinterLy(curr_dim) for _ in range(nstack - 1)
        ])
        ...
        self.tl_regrs = nn.ModuleList([
            mkregrLy(cnv_dim, curr_dim, 2) for _ in range(nstack)
        ])
